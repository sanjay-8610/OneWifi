...
/* Temporary storage for assoc request data (captured on req, used on resp) */
typedef struct {
    mac_addr_str_t mac_str;
    int ap_index;
    int sig_dbm;
    int sub_event;              /* wifi_event_hal_assoc_req_frame or reassoc */
    struct timespec timestamp;  /* Capture time (CLOCK_REALTIME) */
    uint8_t vendor_ie_count;
    uint16_t ie_data_len;
    uint8_t ie_data[MAX_STATS_IE_DATA_LEN]; /* TLV-encoded IEs from Assoc Request */
} lq_assoc_req_store_t;

/* Maximum pending assoc requests to track */
#define MAX_LQ_ASSOC_REQ_STORE 10
...
typedef struct {
    ...
    /* Pending assoc request store: keyed by MAC, used on successful response */
    hash_map_t *assoc_req_store;
    pthread_mutex_t assoc_req_lock;
