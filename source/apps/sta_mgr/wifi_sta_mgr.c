...
void process_dhcp_packet(wifi_app_t *app, const uint8_t *buffer, ssize_t len)
{
    ...
    if ((dhcp->op == DHCP_BOOTP)) {
        if ((options[2] == DHCP_OP_DISCOVER_INDEX) || (options[2] == DHCP_OP_REQUEST_INDEX)) {
            ...
            memset(vendor_class_id, 0, sizeof(vendor_class_id));
            ...
            while (options_len > 0) {
                ...
                if (option_type == DHCP_OPTION_HOSTNAME) {
                    if (current_op_len < sizeof(hostname)) {
                        memcpy(hostname, &options[2], current_op_len);
                    } else {
                        memcpy(hostname, &options[2], sizeof(hostname) - 1);
                    ...
                    if (current_op_len < sizeof(vendor_class_id)) {
                        memcpy(vendor_class_id, &options[2], current_op_len);
                    } else {
                        memcpy(vendor_class_id, &options[2], sizeof(vendor_class_id) - 1);
                        vendor_class_id[sizeof(vendor_class_id) - 1] = '\0';
                    }
                }
...
void *dhcp_sniff_event_func(void *arg)
{
    ...
    //TBD: Getting the bridge associated with private 5G VAP. This needs to be changed to
    //get from appropriate vapname.
    bridge_name[0] = '\0';
    ret = get_bridgename_from_vapname(wifi_prop, "private_ssid_5g", bridge_name, sizeof(bridge_name));
    if (ret != RETURN_OK) {
        wifi_util_info_print(WIFI_APPS, "%s:%d: Getting bridge name failed\n", __func__, __LINE__);
        close(raw_socket);
        return 1;
    } else {
        wifi_util_info_print(WIFI_APPS, "%s:%d: Bridge name %s\n", __func__, __LINE__, bridge_name);
    }
